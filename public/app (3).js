...
function updateHeaderStatus() {
    ...
    if (!ct) {
        ...
        setHeaderTitle('فایل‌های ذخیره‌شده');
        headerStatus.innerHTML = `<span class="dot"></span><span>${STATE.savedFiles.length} فایل</span>`;
    } else {
...
function chatNameFor(id) {
    ...
    if (id === FILES_ID) return 'فایل‌های ذخیره‌شده';
...
function renderMessages() {
    ...
    // Saved Files view — locally saved files
    if (STATE.privateChatWith === FILES_ID) {
        renderSavedFiles();
        return;
    }
...
// ---- Saved files (local browser) ----
function loadSavedFiles() {
    try {
        const local = localStorage.getItem('pm_files_' + STATE.myId);
        STATE.savedFiles = local ? JSON.parse(local) : [];
    } catch (e) { STATE.savedFiles = []; }
    renderMessages();
}
function saveFilesLocal() {
    try { localStorage.setItem('pm_files_' + STATE.myId, JSON.stringify(STATE.savedFiles)); } catch (e) {}
}
function renderSavedFiles() {
    if (!STATE.savedFiles.length) {
        chatEl.innerHTML = emptyHtml('📁', 'فایل ذخیره‌شده‌ای نیست', 'برای ذخیره، روی پیام راست‌کلیک کنید → ذخیره');
        return;
    }
    let html = '';
    STATE.savedFiles.forEach(f => {
        const time = formatTime(f.saved_at || Date.now());
        let preview = '';
        if (f.type === 'image') preview = `<div class="bubble-media"><img src="${f.url}" alt="" loading="lazy" onclick="window.open('${f.url}','_blank')"></div>`;
        else if (f.type === 'video') preview = `<div class="bubble-media"><video controls src="${f.url}"></video></div>`;
        else if (f.type === 'audio') preview = `<div class="bubble-media"><audio controls src="${f.url}"></audio></div>`;
        html += `
        <div class="msg-row other">
            <div class="msg-avatar" style="background:#a890b8;">📁</div>
            <div class="bubble">
                <span class="bubble-name" style="color:#a890b8;">${escapeHtml(f.name || 'فایل')}</span>
                ${preview}
                ${f.text ? `<span class="bubble-text">${escapeHtml(f.text)}</span>` : ''}
...
function ctxSave() {
    ...
    if (!msg) return;
    const item = {
        name: msg.media ? (msg.media.name || 'فایل') : (msg.username || 'کاربر'),
        url: msg.media ? msg.media.url : null,
        type: msg.media ? msg.media.type : null,
        text: msg.message || (msg.sticker || ''),
        saved_at: new Date().toISOString()
    };
    STATE.savedFiles.unshift(item);
    saveFilesLocal();
    ctxMenu.classList.add('hidden');
    showToast('💾 ذخیره شد', '💾');
    if (STATE.chatType === 'files') renderMessages();
}
...
function renderChatList() {
    ...
    items.push({ id: FILES_ID, type: 'files', name: 'فایل‌های ذخیره‌شده', icon: '📁', color: '#a890b8', lastMsg: 'فایل‌های شما' });
