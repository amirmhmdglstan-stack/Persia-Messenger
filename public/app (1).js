...
// Safe, minimal markdown renderer for AI (همیار) answers
...
function connectSocket() {
    ...
    STATE.socket.on('new_message', (msg) => {
        ...
        if (msg.is_private && msg.recipient_id === STATE.myId && !STATE.privateChatWith) {
            showNotifCard(msg);
        }
    });
    ...
    STATE.socket.on('contacts_update', (list) => {
        STATE.contacts = list || [];
        if (!$('chat-list-modal').classList.contains('hidden')) updateContacts();
    });
...
function updateHeaderStatus() {
    if (STATE.privateChatWith) {
        headerStatus.innerHTML = `<span class="dot"></span><span>چت خصوصی</span>`;
    } else {
...
function renderMessages() {
    ...
    if (STATE.privateChatWith) {
        ...
        if (filtered.length === 0) {
            if (STATE.privateChatWith === HAMYAR_ID && STATE.hamyarTyping) {
    ...
    if (!STATE.privateChatWith && STATE.globalAItyping) html += hamyarTypingHtml();
    chatEl.innerHTML = html;
...
function buildBubble(msg) {
    ...
    if (deleted) {
        ...
        if (msg.message) content += `<span class="bubble-text">${msg.sender_id === HAMYAR_ID ? renderAIMarkdown(msg.message) : escapeHtml(msg.message)}</span>`;
...
function sendToSocket(msg) {
    if (STATE.socket && STATE.socket.connected) {
        ...
        const triggersAI = msg.is_private && msg.recipient_id === HAMYAR_ID ||
            (!msg.is_private && (isHamyarMsg(msg.message) || msg.reply_to));
...
function openPrivateChat(uid, username) {
    STATE.privateChatWith = uid;
    headerTitle.textContent = `💬 ${username || 'کاربر'}`;
    updateHeaderStatus();
    if (uid === SAVED_ID) loadSavedMessages();
    renderMessages();
...
window.openChatList = function() {
    ...
    if (STATE.socket) STATE.socket.emit('get_contacts');
    updateContacts();
};
...
function updateContacts() {
    ...
    if (!STATE.contacts || STATE.contacts.length === 0) {
        container.innerHTML =
            `<div style="text-align:center;color:var(--text-muted);padding:30px;"><div style="font-size:40px;margin-bottom:10px;">👥</div><div>هنوز با کسی چت نکرده‌اید</div><div style="font-size:12px;margin-top:4px;">برای شروع، در Global chat پیام بفرستید یا روی آواتار کاربران کلیک کنید</div></div>`;
        return;
    }
    const sorted = [...STATE.contacts].sort((a, b) => new Date(b.last_time) - new Date(a.last_time));
    container.innerHTML = sorted.map(contact => `
        <div style="display:flex;align-items:center;gap:12px;padding:10px 14px;border-radius:var(--radius);background:var(--bg-input);border:1px solid var(--border);margin-bottom:6px;cursor:pointer;transition:var(--transition);"
             ...
             onclick="startChatWith('${contact.contact_id}','${escapeHtml(contact.username)}')">
            ...
            <div style="flex:1;min-width:0;">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    ...
                    <span style="font-size:10px;color:var(--text-muted);">${formatTime(contact.last_time)}</span>
                </div>
                <div style="font-size:12px;color:var(--text-muted);overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">
                    ${contact.last_message ? truncate(contact.last_message, 30) : '...'}
                </div>
            </div>
        </div>
    `).join('');
