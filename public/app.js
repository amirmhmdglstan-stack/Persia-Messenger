...
function openPrivateChat(uid, username) {
    ...
    // "Clear full history" button — only for the همیار bot contact
    if (uid === HAMYAR_ID && !document.querySelector('.clear-bot-btn')) {
    ...
    } else if (uid !== HAMYAR_ID) {
        const old = document.querySelector('.clear-bot-btn');
        if (old) old.remove();
    }
    scrollToBottom();
}
