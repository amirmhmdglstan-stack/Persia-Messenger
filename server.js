...
async function handleVideoRequest(uid, socket, newMsg){
    ...
    if (urls.length) {
        ...
        // ---- Method 1: yt-dlp (works for many sites; YouTube often blocked on cloud) ----
        const j = await runYtdlpJson(url);
        if (j && !j.error && j.formats) {
        ...
        // ---- Method 2: Cobalt community API (free, no key; good for YouTube/social) ----
        reply(socket, uid, '🎬 در حال تلاش برای دانلود... (روش‌های مختلف را امتحان می‌کنم)', true);
        const cobalt = await tryCobalt(url);
        if (cobalt) {
            await sendFromUrl(socket, uid, cobalt, 'cobalt');
            return;
        }

        // ---- Method 3: optional keyed APIs (set env vars to enable) ----
        if (process.env.YT_API_URL && process.env.YT_API_KEY) {
            const keyed = await tryKeyedApi(url, process.env.YT_API_URL, process.env.YT_API_KEY);
            if (keyed) { await sendFromUrl(socket, uid, keyed, 'api'); return; }
        }

        reply(socket, uid, '❌ هیچ روش رایگانی نتوانست این ویدیو را دانلود کند. یوتیوب اغلب سرورهای ابری را مسدود می‌کند. لطفاً یک لینک مستقیم (.mp4 / .mp3 / ...) بفرستید.');
        return;
    }
    reply(socket, uid, '🎬 لینک ویدیو (یوتیوب و...) یا لینک مستقیم فایل را بفرستید.');
}

// ---- Cobalt community API (free download) ----
// Override/extend via env var COBALT_INSTANCES (comma-separated URLs).
const COBALT_INSTANCES = (process.env.COBALT_INSTANCES || [
    'https://cobalt-api.kwiatekmiki.com',
    'https://cobalt.kwiatekmiki.com',
    'https://co.eepy.lol',
    'https://cobalt.wolf-yuan.dev',
    'https://api.cobalt.ceo',
    'https://cobalt-backend.pico11.dev',
    'https://dl.kvsss.top',
    'https://cobalt.aerialys.pw',
    'https://cobalt.147600.xyz',
].join(',')).split(',').map(s=>s.trim()).filter(Boolean);
async function tryCobalt(url){
    for (const inst of COBALT_INSTANCES) {
        try {
            const r = await fetch(inst + '/api/json', {
                method:'POST',
            ...
            if (d && (d.status==='success' || d.status==='tunnel') && d.url) return d.url;
        } catch(e){ continue; }
    }
    return null;
}
...
// ---- optional keyed API ----
async function tryKeyedApi(url, apiUrl, apiKey){
    try {
        ...
        if (!r.ok) return null;
        const d = await r.json().catch(()=>null);
        if (d && (d.url || d.download_url || d.file)) return d.url || d.download_url || d.file;
    } catch(e){ return null; }
    return null;
}

// fetch a media URL and send it back as a message
async function sendFromUrl(socket, uid, url, source){
    try {
        const res = await fetch(url, { signal: AbortSignal.timeout(120000) });
        if (!res.ok) { reply(socket, uid, '❌ دانلود از ' + source + ' ناموفق بود (کد ' + res.status + ')'); return false; }
        const blob = await res.arrayBuffer();
        const mime = res.headers.get('content-type') || '';
        const pathUrl = new URL(url);
        const pname = pathUrl.pathname.split('/').pop() || 'file';
        const ext = (pname.match(/\.([a-z0-9]+)(\?.*)?$/i) || [,'bin'])[1];
        const name = 'dl_' + Date.now() + '.' + (ext.length<=5?ext:'bin');
        const dest = path.join(uploadDir, name);
        fs.writeFileSync(dest, Buffer.from(blob));
        const type = mime.startsWith('video')?'video':(mime.startsWith('image')?'image':(mime.startsWith('audio')?'audio':'file'));
        reply(socket, uid, '🎬 دانلود شد! (' + source + ')', true);
        sendMedia(socket, uid, dest, type, ext);
        return true;
    } catch(e){ reply(socket, uid, '❌ دانلود از ' + source + ' ناموفق بود.'); return false; }
}
    ...
    return new Promise((resolve)=>{
        try {
            ...
            execFile(YTDLP_PATH, ['--dump-json','--no-playlist', url], { timeout: 30000 }, (err, stdout, stderr) => {
                if(err){ resolve({ error: 'missing_or_crash', detail: String(stderr||err.message).slice(0,200) }); return; }
                if(!stdout){ resolve({ error: 'no_output' }); return; }
                try { const j = JSON.parse(stdout.split('\n')[0]); resolve(j); } catch(e){ resolve({ error: 'parse', detail: String(stdout).slice(0,200) }); }
            });
        } catch(e){ resolve({ error: 'missing' }); }
    });
}
...
function isSpam(socketId, msg) {
    // Bot interactions (video downloader, AI) are intentional single sends —
    // never block them, even if the user sends several in a row.
    const toBot = msg.is_private && (msg.recipient_id === VID_ID || msg.recipient_id === HAMYAR_ID);
    if (toBot) return false;
    const now = Date.now();
